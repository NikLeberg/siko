...
/* CANopen-Bootup abwarten */
unsigned long sikoCanBootupAwait(siko_typ* s, sikoJob_typ* j) {
    ...
    // Status speichern
    j->parameter = s->can.bootup.recv;
    if (j->parameter) return sikoERR_OK;
    // Warte
    if (j->step) --j->step;
    else return sikoERR_TIMEOUT;
    return ERR_FUB_BUSY;
}
...
unsigned long sikoCanSDOwrite(siko_typ* s, sikoJob_typ* j) {
    // SDO Parameter vorbereiten
    if (!s->can.sdoWrite.enable) {
        ...
        plcstring* str = (plcstring*) j->parameter;
